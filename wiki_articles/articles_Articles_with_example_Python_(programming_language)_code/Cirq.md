# Cirq

## Article Metadata

- **Last Updated:** 2024-12-14T19:34:27.000414+00:00
- **Original Article:** [Cirq](https://en.wikipedia.org/wiki/Cirq)
- **Language:** en
- **Page ID:** 60102841

## Summary

Cirq is an open-source framework for noisy intermediate scale quantum (NISQ) computers.

## Categories

- Category:Articles with example Python (programming language) code
- Category:Articles with short description
- Category:Quantum programming
- Category:Short description is different from Wikidata

## Table of Contents

- History
- Usage
- Projects

## Content

Cirq is an open-source framework for noisy intermediate scale quantum (NISQ) computers.

History
Cirq was developed by the Google AI Quantum Team, and the public alpha was announced at the International Workshop on Quantum Software and Quantum Machine Learning on July 18, 2018.  A demo by QC Ware showed an implementation of QAOA solving an example of the maximum cut problem being solved on a Cirq simulator.

Usage
Quantum programs in Cirq are represented by "Circuit" which is made up of a series of "Moments" representing slices of quantum gates that should be applied at the same time. The programs can be executed on local simulators or against hardware supplied by IonQ, Pasqal, Rigetti, and Alpine Quantum Technologies
The following example shows how to create and measure a Bell state in Cirq.

Printing the circuit displays its diagram

Simulating the circuit repeatedly shows that the measurements of the qubits are correlated.

Projects
OpenFermion
OpenFermion is a library that compiles quantum simulation algorithms to Cirq.

TensorFlow Quantum
TensorFlow Quantum is an extension of TensorFlow that allows TensorFlow to be used to explore hybrid classical-quantum machine learning algorithms.

ReCirq
ReCirq is a repository of research projects done using Cirq.

Qsim Cirq
Qsim is a high performance wave function simulator that leverages gate fusing, AVS/FMA instructions, and OpenMP to achieve fast simulation rates. Qsimcirq allows one to use qsim from within Cirq.


== References ==

## Related Articles

### Internal Links

- [Adiabatic quantum computation](https://en.wikipedia.org/wiki/Adiabatic_quantum_computation)
- [Amplitude amplification](https://en.wikipedia.org/wiki/Amplitude_amplification)
- [ArXiv](https://en.wikipedia.org/wiki/ArXiv)
- [BB84](https://en.wikipedia.org/wiki/BB84)
- [BHT algorithm](https://en.wikipedia.org/wiki/BHT_algorithm)
- [BQP](https://en.wikipedia.org/wiki/BQP)
- [Bacon–Shor code](https://en.wikipedia.org/wiki/Bacon%E2%80%93Shor_code)
- [Bell's theorem](https://en.wikipedia.org/wiki/Bell%27s_theorem)
- [Bell state](https://en.wikipedia.org/wiki/Bell_state)
- [Bernstein–Vazirani algorithm](https://en.wikipedia.org/wiki/Bernstein%E2%80%93Vazirani_algorithm)
- [Bibcode](https://en.wikipedia.org/wiki/Bibcode)
- [Boson sampling](https://en.wikipedia.org/wiki/Boson_sampling)
- [CSS code](https://en.wikipedia.org/wiki/CSS_code)
- [Cavity quantum electrodynamics](https://en.wikipedia.org/wiki/Cavity_quantum_electrodynamics)
- [Charge qubit](https://en.wikipedia.org/wiki/Charge_qubit)
- [Circuit quantum electrodynamics](https://en.wikipedia.org/wiki/Circuit_quantum_electrodynamics)
- [Classical capacity](https://en.wikipedia.org/wiki/Classical_capacity)
- [Cloud-based quantum computing](https://en.wikipedia.org/wiki/Cloud-based_quantum_computing)
- [Cluster state](https://en.wikipedia.org/wiki/Cluster_state)
- [Continuous-variable quantum information](https://en.wikipedia.org/wiki/Continuous-variable_quantum_information)
- [Cross-entropy benchmarking](https://en.wikipedia.org/wiki/Cross-entropy_benchmarking)
- [Deutsch–Jozsa algorithm](https://en.wikipedia.org/wiki/Deutsch%E2%80%93Jozsa_algorithm)
- [DiVincenzo's criteria](https://en.wikipedia.org/wiki/DiVincenzo%27s_criteria)
- [Digital object identifier](https://en.wikipedia.org/wiki/Digital_object_identifier)
- [Eastin–Knill theorem](https://en.wikipedia.org/wiki/Eastin%E2%80%93Knill_theorem)
- [Entanglement-assisted classical capacity](https://en.wikipedia.org/wiki/Entanglement-assisted_classical_capacity)
- [Entanglement-assisted stabilizer formalism](https://en.wikipedia.org/wiki/Entanglement-assisted_stabilizer_formalism)
- [Entanglement distillation](https://en.wikipedia.org/wiki/Entanglement_distillation)
- [Exact quantum polynomial time](https://en.wikipedia.org/wiki/Exact_quantum_polynomial_time)
- [Flux qubit](https://en.wikipedia.org/wiki/Flux_qubit)
- [GitHub](https://en.wikipedia.org/wiki/GitHub)
- [Gleason's theorem](https://en.wikipedia.org/wiki/Gleason%27s_theorem)
- [Gnu code](https://en.wikipedia.org/wiki/Gnu_code)
- [Gottesman–Knill theorem](https://en.wikipedia.org/wiki/Gottesman%E2%80%93Knill_theorem)
- [Grover's algorithm](https://en.wikipedia.org/wiki/Grover%27s_algorithm)
- [HHL algorithm](https://en.wikipedia.org/wiki/HHL_algorithm)
- [Hidden subgroup problem](https://en.wikipedia.org/wiki/Hidden_subgroup_problem)
- [Holevo's theorem](https://en.wikipedia.org/wiki/Holevo%27s_theorem)
- [IBM Quantum Platform](https://en.wikipedia.org/wiki/IBM_Quantum_Platform)
- [IonQ](https://en.wikipedia.org/wiki/IonQ)
- [KLM protocol](https://en.wikipedia.org/wiki/KLM_protocol)
- [Kane quantum computer](https://en.wikipedia.org/wiki/Kane_quantum_computer)
- [LOCC](https://en.wikipedia.org/wiki/LOCC)
- [Libquantum](https://en.wikipedia.org/wiki/Libquantum)
- [Linear optical quantum computing](https://en.wikipedia.org/wiki/Linear_optical_quantum_computing)
- [List of quantum key distribution protocols](https://en.wikipedia.org/wiki/List_of_quantum_key_distribution_protocols)
- [List of quantum processors](https://en.wikipedia.org/wiki/List_of_quantum_processors)
- [Maximum cut](https://en.wikipedia.org/wiki/Maximum_cut)
- [Model of computation](https://en.wikipedia.org/wiki/Model_of_computation)
- [Monogamy of entanglement](https://en.wikipedia.org/wiki/Monogamy_of_entanglement)
- [Noisy intermediate-scale quantum era](https://en.wikipedia.org/wiki/Noisy_intermediate-scale_quantum_era)
- [Neutral atom quantum computer](https://en.wikipedia.org/wiki/Neutral_atom_quantum_computer)
- [Nitrogen-vacancy center](https://en.wikipedia.org/wiki/Nitrogen-vacancy_center)
- [No-broadcasting theorem](https://en.wikipedia.org/wiki/No-broadcasting_theorem)
- [No-cloning theorem](https://en.wikipedia.org/wiki/No-cloning_theorem)
- [No-communication theorem](https://en.wikipedia.org/wiki/No-communication_theorem)
- [No-deleting theorem](https://en.wikipedia.org/wiki/No-deleting_theorem)
- [No-hiding theorem](https://en.wikipedia.org/wiki/No-hiding_theorem)
- [No-teleportation theorem](https://en.wikipedia.org/wiki/No-teleportation_theorem)
- [Noisy intermediate-scale quantum era](https://en.wikipedia.org/wiki/Noisy_intermediate-scale_quantum_era)
- [Nuclear magnetic resonance quantum computer](https://en.wikipedia.org/wiki/Nuclear_magnetic_resonance_quantum_computer)
- [One-way quantum computer](https://en.wikipedia.org/wiki/One-way_quantum_computer)
- [Open source](https://en.wikipedia.org/wiki/Open_source)
- [OpenQASM](https://en.wikipedia.org/wiki/OpenQASM)
- [Pusey–Barrett–Rudolph theorem](https://en.wikipedia.org/wiki/Pusey%E2%80%93Barrett%E2%80%93Rudolph_theorem)
- [PubMed Central](https://en.wikipedia.org/wiki/PubMed_Central)
- [PubMed](https://en.wikipedia.org/wiki/PubMed)
- [Phase qubit](https://en.wikipedia.org/wiki/Phase_qubit)
- [Physical and logical qubits](https://en.wikipedia.org/wiki/Physical_and_logical_qubits)
- [Post-quantum cryptography](https://en.wikipedia.org/wiki/Post-quantum_cryptography)
- [PostBQP](https://en.wikipedia.org/wiki/PostBQP)
- [Python (programming language)](https://en.wikipedia.org/wiki/Python_(programming_language))
- [QC Ware](https://en.wikipedia.org/wiki/QC_Ware)
- [QIP (complexity)](https://en.wikipedia.org/wiki/QIP_(complexity))
- [QMA](https://en.wikipedia.org/wiki/QMA)
- [Q Sharp](https://en.wikipedia.org/wiki/Q_Sharp)
- [Qiskit](https://en.wikipedia.org/wiki/Qiskit)
- [Quantum Fourier transform](https://en.wikipedia.org/wiki/Quantum_Fourier_transform)
- [Quantum Turing machine](https://en.wikipedia.org/wiki/Quantum_Turing_machine)
- [Quantum algorithm](https://en.wikipedia.org/wiki/Quantum_algorithm)
- [Quantum annealing](https://en.wikipedia.org/wiki/Quantum_annealing)
- [Quantum capacity](https://en.wikipedia.org/wiki/Quantum_capacity)
- [Quantum channel](https://en.wikipedia.org/wiki/Quantum_channel)
- [Quantum circuit](https://en.wikipedia.org/wiki/Quantum_circuit)
- [Quantum coin flipping](https://en.wikipedia.org/wiki/Quantum_coin_flipping)
- [Quantum complexity theory](https://en.wikipedia.org/wiki/Quantum_complexity_theory)
- [Quantum computing](https://en.wikipedia.org/wiki/Quantum_computing)
- [Quantum convolutional code](https://en.wikipedia.org/wiki/Quantum_convolutional_code)
- [Quantum counting algorithm](https://en.wikipedia.org/wiki/Quantum_counting_algorithm)
- [Quantum cryptography](https://en.wikipedia.org/wiki/Quantum_cryptography)
- [Quantum error correction](https://en.wikipedia.org/wiki/Quantum_error_correction)
- [Quantum gate teleportation](https://en.wikipedia.org/wiki/Quantum_gate_teleportation)
- [Quantum information](https://en.wikipedia.org/wiki/Quantum_information)
- [Quantum information science](https://en.wikipedia.org/wiki/Quantum_information_science)
- [Quantum key distribution](https://en.wikipedia.org/wiki/Quantum_key_distribution)
- [Quantum logic gate](https://en.wikipedia.org/wiki/Quantum_logic_gate)
- [Quantum machine learning](https://en.wikipedia.org/wiki/Quantum_machine_learning)
- [Quantum money](https://en.wikipedia.org/wiki/Quantum_money)
- [Quantum network](https://en.wikipedia.org/wiki/Quantum_network)
- [Quantum neural network](https://en.wikipedia.org/wiki/Quantum_neural_network)
- [Quantum optics](https://en.wikipedia.org/wiki/Quantum_optics)
- [Quantum optimization algorithms](https://en.wikipedia.org/wiki/Quantum_optimization_algorithms)
- [Quantum phase estimation algorithm](https://en.wikipedia.org/wiki/Quantum_phase_estimation_algorithm)
- [Quantum programming](https://en.wikipedia.org/wiki/Quantum_programming)
- [Quantum secret sharing](https://en.wikipedia.org/wiki/Quantum_secret_sharing)
- [Quantum simulator](https://en.wikipedia.org/wiki/Quantum_simulator)
- [Quantum speed limit](https://en.wikipedia.org/wiki/Quantum_speed_limit)
- [Quantum supremacy](https://en.wikipedia.org/wiki/Quantum_supremacy)
- [Quantum teleportation](https://en.wikipedia.org/wiki/Quantum_teleportation)
- [Quantum volume](https://en.wikipedia.org/wiki/Quantum_volume)
- [Qubit](https://en.wikipedia.org/wiki/Qubit)
- [Quil (instruction set architecture)](https://en.wikipedia.org/wiki/Quil_(instruction_set_architecture))
- [Randomized benchmarking](https://en.wikipedia.org/wiki/Randomized_benchmarking)
- [Relaxation (NMR)](https://en.wikipedia.org/wiki/Relaxation_(NMR))
- [Rigetti Computing](https://en.wikipedia.org/wiki/Rigetti_Computing)
- [SARG04](https://en.wikipedia.org/wiki/SARG04)
- [Schrödinger–HJW theorem](https://en.wikipedia.org/wiki/Schr%C3%B6dinger%E2%80%93HJW_theorem)
- [Shor's algorithm](https://en.wikipedia.org/wiki/Shor%27s_algorithm)
- [Quantum error correction](https://en.wikipedia.org/wiki/Quantum_error_correction)
- [Simon's problem](https://en.wikipedia.org/wiki/Simon%27s_problem)
- [Software development](https://en.wikipedia.org/wiki/Software_development)
- [Software framework](https://en.wikipedia.org/wiki/Software_framework)
- [Software license](https://en.wikipedia.org/wiki/Software_license)
- [Solovay–Kitaev theorem](https://en.wikipedia.org/wiki/Solovay%E2%80%93Kitaev_theorem)
- [Spin (physics)](https://en.wikipedia.org/wiki/Spin_(physics))
- [Spin qubit quantum computer](https://en.wikipedia.org/wiki/Spin_qubit_quantum_computer)
- [Spin–lattice relaxation](https://en.wikipedia.org/wiki/Spin%E2%80%93lattice_relaxation)
- [Spin–spin relaxation](https://en.wikipedia.org/wiki/Spin%E2%80%93spin_relaxation)
- [Stabilizer code](https://en.wikipedia.org/wiki/Stabilizer_code)
- [Steane code](https://en.wikipedia.org/wiki/Steane_code)
- [Superconducting quantum computing](https://en.wikipedia.org/wiki/Superconducting_quantum_computing)
- [Superdense coding](https://en.wikipedia.org/wiki/Superdense_coding)
- [TensorFlow](https://en.wikipedia.org/wiki/TensorFlow)
- [Threshold theorem](https://en.wikipedia.org/wiki/Threshold_theorem)
- [Timeline of quantum computing and communication](https://en.wikipedia.org/wiki/Timeline_of_quantum_computing_and_communication)
- [Topological quantum computer](https://en.wikipedia.org/wiki/Topological_quantum_computer)
- [Toric code](https://en.wikipedia.org/wiki/Toric_code)
- [Transmon](https://en.wikipedia.org/wiki/Transmon)
- [Trapped-ion quantum computer](https://en.wikipedia.org/wiki/Trapped-ion_quantum_computer)
- [Ultracold atom](https://en.wikipedia.org/wiki/Ultracold_atom)
- [Variational quantum eigensolver](https://en.wikipedia.org/wiki/Variational_quantum_eigensolver)
- [Template:Quantum information](https://en.wikipedia.org/wiki/Template:Quantum_information)
- [Template:Quantum mechanics topics](https://en.wikipedia.org/wiki/Template:Quantum_mechanics_topics)
- [Template talk:Quantum information](https://en.wikipedia.org/wiki/Template_talk:Quantum_information)
- [Category:Quantum information science](https://en.wikipedia.org/wiki/Category:Quantum_information_science)

---
_This article is part of the Python Programming Language wiki archive._
_Retrieved and archived on: 2024-12-14T19:34:27.000414+00:00_